...
def get_referCase(id):
    ...
    caserefers = db.session.query(Caserefer.id, Caserefer.mockid, Caserefer.refer_mockid, Caserefer.ordernum, Testcase.case_name).filter_by(mockid=id).order_by(db.asc(Caserefer.ordernum)).join(
        Testcase, Caserefer.refer_mockid == Testcase.id)

    return jsonify({
        'code': 1,
        'caserefers': [{'id': caserefer.id, 'mockid': caserefer.mockid, 'refer_mockid': caserefer.refer_mockid, 'ordernum': caserefer.ordernum, 'refer_casename': caserefer.case_name} for
